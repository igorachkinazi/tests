    ...
    public:
        string add(string x, string y) { 
            if(isNumeric(x) && isNumeric(y))
                return "0"; //to_string(to_int(x)+to_int(y));
            if(!isNumeric(x) && !isNumeric(y))
                return "2";
            return "1";
        } 
        ...
        string mul(string x, string y) {
            if(isNumeric(x) && isNumeric(y))
                return to_string(to_int(x)*to_int(y));
            return "("+x+"*"+y+")";
        }
...
string evaluate(BinaryTree *parseTree) {
    ...
    if (leftC && rightC) {
        if (parseTree->getRootVal() == "+") {
            return Oper.add(evaluate(leftC), evaluate(rightC));
...
int main() {
    ...
    BinaryTree *pt = buildParseTree("((10+x)*3)");
    postorder(pt);
    cout << endl << "Evaluate: ";
