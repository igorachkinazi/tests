...
int main() {
    ...
    vector<string> words(arr, arr + numwords);

    // Graph g = buildGraph(words);
    Graph g(false);

    for(int i = 0; i < numwords; i++) {
        cout << "\nShortest paths from " << arr[i] << ":\n";
        for(int j = 0; j < numwords; j++) {
            if(i == j)
                continue;

            g = buildGraph(words);
            g = bfs(g, g.getVertex(arr[i]));
            cout << "To " << arr[j] << ":" << distance(g.getVertex(arr[j])) << endl;
        }
    }
