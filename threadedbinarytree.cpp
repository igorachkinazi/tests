        ...
        void _put(int key, string val, TreeNode *currentNode){
            ...
            if (key < currentNode->key){
                if (currentNode->hasLeftChild()){
                    ...
                    new_node = currentNode->leftChild = new TreeNode(key, val, currentNode);
                }
            }
            else{
                if (currentNode->hasRightChild()){
                    ...
                    new_node =currentNode->rightChild = new TreeNode(key, val, currentNode);
                }
            }
            if(new_node) {
                _setPrevThread(new_node);
                new_node->threadNext = new_node->findSuccessor();
            }
        }

        void _setPrevThread(TreeNode *current_node){
            TreeNode *node = root->findMin();
            if(node == current_node)
                return;
            while(node) {
                TreeNode *succ = node->findSuccessor();
                if(succ == current_node) {
                   node->threadNext = current_node;
                   return;
                }
                node = succ;
            }
