...
public:
    ...
    void dfs(){
        ...
        std::stack<vertex_t> vstack;

        for(auto& cur : vertices){
            if(std::find(visitedList.begin(), visitedList.end(), cur.first) == visitedList.end()){
                dfsvisit(visitedList, cur.second, vstack);
            }
        }

        std::vector<vertex_t> vtopology;
        while (!vstack.empty()) {
           vtopology.push_back(vstack.top());
           vstack.pop();
        }

        std::cout << std::endl << "Topology: ";
        for(auto& v : vtopology)
            std::cout << v.first << " ";
        std::cout << std::endl;
    }

    void dfsvisit(std::list<int>& visitedList, vertex_t& vertex, std::stack<vertex_t>& vstack){
        ...
        for(int neighborID : vertex.second){
            if(std::find(visitedList.begin(), visitedList.end(), neighborID) == visitedList.end()){
                dfsvisit(visitedList, getVertex(neighborID), vstack);
            }
        }
        vstack.push(vertex);
     }
